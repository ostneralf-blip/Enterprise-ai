...
describe('Accessibility: Roadmap-Generator', () => {
  ...
  it('Starter-Roadmap hat keine WCAG-Verstöße (axe-core)', async () => {
    const { container } = render(<RoadmapPageClient initialArchetype="starter" fromAssessment={false} tier="free" topUseCases={[]} savedRoadmap={null} />)
    expect(await axe(container)).toHaveNoViolations()
  })

  it('Scaler-Roadmap hat keine WCAG-Verstöße (axe-core)', async () => {
    const { container } = render(<RoadmapPageClient initialArchetype="scaler" fromAssessment={false} tier="free" topUseCases={[]} savedRoadmap={null} />)
    expect(await axe(container)).toHaveNoViolations()
  })

  it('Transformer-Roadmap hat keine WCAG-Verstöße (axe-core)', async () => {
    const { container } = render(<RoadmapPageClient initialArchetype="transformer" fromAssessment={false} tier="free" topUseCases={[]} savedRoadmap={null} />)
    expect(await axe(container)).toHaveNoViolations()
  })

  it('Archetyp-Buttons haben aria-pressed', () => {
    render(<RoadmapPageClient initialArchetype="starter" fromAssessment={false} tier="free" topUseCases={[]} savedRoadmap={null} />)
  ...
  it('aktiver Archetyp-Button hat aria-pressed="true"', () => {
    render(<RoadmapPageClient initialArchetype="scaler" fromAssessment={false} tier="free" topUseCases={[]} savedRoadmap={null} />)
  ...
  it('Assessment-Hinweis wird angezeigt wenn fromAssessment=true', () => {
    render(<RoadmapPageClient initialArchetype="transformer" fromAssessment={true} tier="free" topUseCases={[]} savedRoadmap={null} />)
