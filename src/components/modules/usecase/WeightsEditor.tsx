...
export function WeightsEditor({ weights, onSave, onClose }: WeightsEditorProps) {
  ...
  const handleSave = async () => {
    if (!valid) { setError(t('weightsError')); return }
    setSaving(true)
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 mb-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base sm:text-lg font-semibold text-slate-900">{t('weightsTitle')}</h2>
        <button onClick={onClose} className="text-slate-400 hover:text-slate-600 text-xl leading-none" aria-label={t('weightsClose')}>×</button>
      </div>
          ...
          <div key={c.id} className="flex items-center gap-3 min-w-0">
            ...
            <div className="flex items-center gap-1 shrink-0">
              <input
                ...
                className="w-16 text-sm text-right border border-slate-200 rounded-lg px-2 py-1 focus:outline-none focus:border-primary-ring"
                aria-label={t('weightsAriaLabel', { label: pick(c.label, locale) })}
              />
      ...
      <div className={`text-sm font-medium mb-3 ${valid ? 'text-emerald-600' : 'text-red-500'}`}>
        {t('weightsTotalLabel')} {total} % {valid ? '✓' : `(${total > 100 ? '+' : ''}${total - 100} %)`}
      </div>
